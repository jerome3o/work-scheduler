...
def build_model(
    ...
    for i, j in product(range(n_workers), range(n_tasks)):
        model += (
            ...
            - worker_task_matrix[i][j]*model_parameters.task_start_vector[j] <= 0
        )
    ...
    for i, j in product(range(n_workers), range(n_tasks)):
        model += (
            ...
            - worker_task_matrix[i][j]*model_parameters.worker_finish_vector[i] <= 0
        )
...
def main():
    ...
    for data_file in data_files:
        ...
        params = build_matrices(work_day)
